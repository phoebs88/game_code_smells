...
PATTERN_ENERGY = "(.*(energy).*)|(.*(battery).*)|(.*(usability).*)|(.*(usage).*)|(.*(useful).*)|(.*(user-friendly).*)|(.*(convenient).*)|(.*(power).*)|(.*(flaw).*)|(.*(issue).*)|(.*(defect).*)|(.*(bug).*)|(.*(efficiency).*)|(.*(efficient).*)|(.*(performance).*)|(.*(freezing).*)|(.*(freeze).*)|(.*(hang).*)|(.*(hanging).*)|(.*(crash).*)|(.*(crashing).*)|(.*(fault).*)|(.*(delay).*)|(.*(lag).*)|(.*(glitch).*)|(.*(refactor).*)|(.*(refactored).*)|(.*(refactoring).*)|(.*(code smell).*)|(.*(anti-pattern).*)|(.*(bad).*)|(.*(problem).*)|(.*(control).*)"
...
def analyze_repo(user, project, retry=120):
    ...
    try:
        ...
        ### do for commiter last date
        #if star>=200
        commito = repo.get_commit(sha=sha)
        date=commito.commit.author.date
        if star>=200 and date>'01-01-2017':
...
@click.argument('input_path', default="popular_desktop_csv_100.csv",
                type=click.Path(dir_okay=False))
@click.argument('output_path', default="results_modified.csv",
                type=click.Path(dir_okay=False))
